...
shinyServer(function(input, output, session) {
  ...
  setalldf <- function(gv,display_sample,reference_samples,use_reference,use_gc_correction,data_type){
    ...
    if(use_gc_correction) { countsFieldName <- "cor" } else { countsFieldName  <- "raw" } 
    ...
    if(num_matches==1) {
        ...
        if(sum(grepl("ann",cols))==1 && sum(grepl("cor",cols))==1 && bitwAnd(data_type,gv$WG_MASK)>0) {
          ...
          if(file_type=="baf") {
            ...
            if(sum(colnames(df)=="raw")==0) {
              ...
              } else if(sum(colnames(df)=="bins")!=0) {
                df$raw <- df$bins
              } else { df$raw <-  1}
            }
            df$Ns <- round(df$N)
            if(sum(colnames(df)=="weighted_var")==0) { df$sd_estimate  <- df$N/sqrt(df$raw) }
            else { df$sd_estimate  <- sqrt(df$weighted_var) }
            df$Nsd <- df$sd_estimate
          } 
            ...
            df$N <- df$N * gv$init_table$n_scale
            if(sum(colnames(df)=="raw")==0) { df$raw  <- 1 }  
            if(sum(colnames(df)=="Ns")==0) { df$Ns <- round(df$N) }
            else { df$Ns <- round(df$Ns) }
          }
          browser()
          if(sum(colnames(df)=="ref_mad")>0  && sum(colnames(df)=="Nsd")==0) { df$Nsd <- df$N * (df$ref_mad/df$ref_median) } # mad = 1.4826*sd so this is about sd
          if(sum(colnames(df)=="ref_mad")==0 && sum(colnames(df)=="Nsd")==0) { df$Nsd  <- df$N/sqrt(df$raw) }
          if(sum(colnames(df)=="sd_estimate")==0) { df$sd_estimate  <- df$Nsd }  
          if(sum(colnames(df)=="Nsd")>0) { df$Nmin <- ifelse(df$N>df$Nsd,df$N-df$Nsd,0)  }
          if(sum(colnames(df)=="Nsd")>0) { df$Nmax<- df$N+df$Nsd }
          if(nrow(df)>0) {df$type <- typeFlag}
      ...
      if(use_reference==TRUE && num_reference_samples>0 &&  bitwAnd(data_type,gv$COUNTS_MASK)>0) {
        ...
        if(sum(idx)>0) {
          ...
          # Make reference
          fapply_2 <- function(fn) { gv$m[[fn]][,countsFieldName] }
          ...
          fapply_mad_per_row <- function(n,m) { v <- m[n,]; mad(v[!is.na(v) & v>0]) }
          ...
          progressdf$inc(amount = 0.1, message = "Pooled reference - ", detail = "applying")
          v <- df[,countsFieldName]
          numerator <- df[,countsFieldName]/median(v[!is.na(v) & v>0]) # median normalise sample
          ratio <- (numerator/denominator)
          df$ref_median <- denominator
          df$ref_mad <-  sapply(1:nrow(reftable),fapply_mad_per_row,m)
          df$N <- (2 * gv$init_table$n_scale * numerator/denominator)
          df$Ns <- round(df$N)
          df$type <- typeFlag
          df$ref <- denominator
  ...
  controlsAll <- reactiveValues(cn_max_val = gv$init_table$cn_max_display_val, 
                                ...
                                gc_correct_aux_target = gv$init_table$gc_correct_aux_target,
                                use_reference_wg = gv$init_table$use_reference_wg,
                                use_reference_targeted = gv$init_table$use_reference_targeted,
                                use_reference_aux_target = gv$init_table$use_reference_aux_target,
                                ...
                                display_aux_target_data = gv$init_table$display_aux_target_data,
                                ...
                                display_data_bitflag = gv$WG_FLAG + gv$TARGETED_FLAG + gv$AUX_TARGET_FLAG
  )
  ...
  set_all_dfs <- reactive({
    if(init_table$first_time_flag==FALSE) {
      controlsAll$display_data_bitflag <- 
        ...
        gv$AUX_TARGET_FLAG * as.integer(controlsAll$display_aux_target_data) +
      ...
      if(bitwAnd(what_to_update,gv$AUX_TARGET_FLAG)!=0 ) {
        df$aux_targetdf <- setalldf(gv,controlsAll$display_sample,controlsAll$reference_samples,controlsAll$use_reference_aux_target,controlsAll$gc_correct_aux_target,gv$AUX_TARGET_FLAG)      
      }
      ...
      df_targeted_and_other <- rbind.fill(df$targeteddf,df$aux_targetdf,df$df_baf,df$df_fusion,df$df_segmentation)
  ...
  if(gv$init_table$have_aux_target) {
    output$display_aux_target_data <- renderUI({ checkboxInput(inputId = "display_aux_target_data", label = "Aux Target",value = gv$init_table$display_aux_target_data) })
    if(gv$init_table$locked==FALSE && gv$init_table$multi_sample_mode==TRUE) {
      output$use_reference_aux_target <- renderUI({ checkboxInput(inputId = "use_reference_aux_target", label = "Dynamic reference for Aux Target",value = gv$init_table$use_reference_aux_target) })
      # output$gc_correct_aux_target <- renderUI({ checkboxInput(inputId = "gc_correct_aux_target", label = "GC correct Off Target",value = gv$init_table$gc_correct_aux_target) })
    }
  }
  if(gv$init_table$locked==FALSE && gv$init_table$multi_sample_mode==TRUE) {
  ...
  observeEvent(input$recalculate_reference,{
    ...
    controlsAll$refresh_table <- 
      ...
      as.integer(controlsAll$use_reference_aux_target) * gv$AUX_TARGET_FLAG
      set_all_dfs()
  })
  ...
  observeEvent(input$display_aux_target_data, { 
    cnb_echo(gv,output,flog.info,paste0("observeEvent(input$display_aux_target_data): = ",input$display_aux_target_data))
    controlsAll$display_aux_target_data <- input$display_aux_target_data 
    if(bitwAnd(controlsAll$pending_updates,gv$AUX_TARGET_FLAG)!=0) {
      controlsAll$refresh_table <- as.integer(controlsAll$display_aux_target_data) * gv$AUX_TARGET_FLAG
    }
    set_all_dfs()
    })
  ...
  observeEvent(input$gc_correct_aux_target, { 
    controlsAll$gc_correct_aux_target <- input$gc_correct_aux_target
    controlsAll$refresh_table <- gv$AUX_TARGET_FLAG
    set_all_dfs()
  })
  ...
  observeEvent(input$use_reference_aux_target, { 
    cnb_echo(gv,output,flog.info,paste0("observeEvent(input$use_reference_aux_target): = ",input$use_reference_aux_target))
    controlsAll$use_reference_aux_target <- input$use_reference_aux_target 
    controlsAll$refresh_table <- gv$AUX_TARGET_FLAG
    set_all_dfs()
  })
  ...
  observeEvent(input$min_probe_coverage, {
    ...
    if(min_probe_coverage != controlsAll$min_probe_coverage){
      ...
      controlsAll$refresh_table <- gv$WG_FLAG + gv$TARGETED_FLAG + gv$AUX_TARGET_FLAG
      set_all_dfs()
    }
  })
  ...
  observeEvent(input$display_sample, {
    ...
    controlsAll$refresh_table <-  gv$WG_FLAG + gv$TARGETED_FLAG + gv$AUX_TARGET_FLAG + gv$BAF_FLAG +gv$FUSION_FLAG + gv$SEGMENTATION_FLAG + gv$ERROR_BARS_FLAG
...
#     cnb_echo(gv,output,flog.info,paste0("displayOptions3$chr = ",displayOptions3$chr))
  ...
  observeEvent(input$plot3_brush,{
    ...
    if(nrow(df)>0) {
      ...
      if(sum(names(displayOptions4$df)=="id")==0) {
          displayOptions4$df$exon <- gv$exons[idx,"exon_number"]
      } else {
        displayOptions4$df$exon <-  ifelse(is.na(displayOptions4$df$id)| grepl("_ex",displayOptions4$df$id),
                                           gv$exons[idx,"exon_number"],"")
      }
      ...
      if(sum(names(colorder)=="id")==1) { colorder["id"] <- end_pos + 0.7 }
      if(sum(names(colorder)=="zscore")==1) { colorder["zscore"] <- end_pos + 0.8 }
      if(sum(names(colorder)=="Nsd")==1) { 
        displayOptions4$df$Nsd <- round(displayOptions4$df$Nsd,2)
        colorder["Nsd"] <- end_pos + 0.9 
      }
  ...
  output$plot1 <- renderPlot({
    ...
    # Display overflow red triangles
  ...
  output$plot2 <- renderPlot({
    ...
    ret <- ggplot(df_counts, aes(x = x, y = N,size=gv$init_tab$plot2_point_size, colour = CN, xlab="", ylab="N")) +
      ...
      xlab("") + 
      ylab("N")
    # Display data points if there are any
    ...
    if(sum(as.integer(idx))>0) {
      df_overflow <- displayOptions2$df[idx,c("x","Ns")]
      df_overflow$Nlim <- displayOptions2$ymax
      df_overflow$text <- as.character(round(df_overflow$Ns,1))
      ret <- ret + annotate("point", x = df_overflow$x, y = df_overflow$Nlim, size = 4, colour = "red", shape = 2) 
      ret <- ret + annotate("text", x = df_overflow$x, y = df_overflow$Nlim + runif(nrow(df_overflow),min = -1,max = 0),label = df_overflow$text)
    }
  ...
  output$plot3 <- renderPlot({
    ...
    # Display overflow red triangles
    idx <- ((displayOptions3$df$N > displayOptions3$ymax) | 
               ((displayOptions3$df$Nmax > displayOptions3$ymax) & controlsAll$display_error_bars)) & 
      (bitwAnd(displayOptions3$df$type,gv$COUNTS_MASK)>0)
    if(sum(as.integer(idx),na.rm = T)>0) {
      df_overflow <- displayOptions3$df[idx,c("x","Ns","Nmax")]
      df_overflow$Nlim <- displayOptions3$ymax
      if(controlsAll$display_error_bars){ Nover <- df_overflow$Nmax } else { Nover <- df_overflow$Ns }
      df_overflow$text <- as.character(round(Nover,1))
      ret <- ret + annotate("point", x = df_overflow$x, y = df_overflow$Nlim, size = 4, colour = "red", shape = 2) 
      ret <- ret + annotate("text", x = df_overflow$x, y = df_overflow$Nlim + runif(nrow(df_overflow),min = -1,max = 0),label = df_overflow$text)
    }
