...
USE_ALT_READ_TABLE <- FALSE # Replace read.table() with fread() here
HEAD_N <- 100 # number of lines to read in to try and guess
...
init_constant_globals <- function(...){
  ...
  gv$cn_min_val <- 0
  gv$resolutions <- c("wg_coarse","wg_medium","wg_fine","targeted","","","","off_target")
  ...
  gv$init_table$have_off_target <- FALSE
...
# For stand-alone debugging outside of the Shiny framework
  ...
  gv$init_table$have_off_target <- sum(grepl(gv$resolutions[gv$OFF_TARGET_FLAG],gv$filelist$resolution) & gv$filelist$file=="counts")>0
